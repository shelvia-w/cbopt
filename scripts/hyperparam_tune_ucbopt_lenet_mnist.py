"""HPC/local uCBOpt tuning sweep for LeNet on MNIST.
...
def run_dir(lr: str, weight_decay: str, hess_init: str, epochs: str = EPOCHS) -> Path:
    return (
        ...
        / OPTIMIZER
        / f"{DATASET}_{MODEL}"
        / f"lr_{lr}_wd_{weight_decay}_h0_{hess_init}_ep_{epochs}"
    )
...
def best_val_metrics(
    save_dir: Path, lr: str, weight_decay: str, hess_init: str
...
def markdown_table(rows: list[dict[str, str | float | int]]) -> str:
    header = "| lr | weight-decay | h0 | best val NLL | best val accuracy | best val ECE | epoch | stage |"
    divider = "|---|---:|---:|---:|---:|---:|---:|---|"
...
def write_summary(
    ...
    recommendation = (
        ...
        f"lr_{best['lr']}_wd_{best['weight_decay']}_h0_{best['hess_init']}_ep_100\"\n"
        "train_args:\n"
        f"  lr: \"{best['lr']}\"\n"
        "  e: \"100\"\n"
        f"  weight-decay: \"{best['weight_decay']}\"\n"
        f"  hess_init: \"{best['hess_init']}\"\n"
        f"  cand_curvature: \"{CAND_CURVATURE}\"\n"
        "  beta1: \"0.9\"\n"
        "  beta2: \"0.99999\"\n"
        "  tbatch: \"128\"\n"
        "  vbatch: \"256\"\n"
...
def main() -> None:
    ...
    def add_row(row: dict[str, str | float | int], stage: str) -> None:
        key = (str(row["lr"]), str(row["weight_decay"]), str(row["hess_init"]))
        if key not in seen:
            seen.add(key)
