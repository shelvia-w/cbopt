...
sys.path.append("..")
from common.baselines.ucbopt import uCBOpt
...
def get_args():
    p = argparse.ArgumentParser(description="uCBOpt + MC Dropout training")

    p.add_argument("arch", choices=MODELS, help="model architecture")
    ...
    p.add_argument("--rescale_lr", action="store_true", help="enable learning-rate rescaling")

    # uCBOpt hyperparams
    p.add_argument("--wd", "--weight-decay", dest="weight_decay", default=1e-4, type=float, help="weight decay")
    p.add_argument("--beta1", default=0.9, type=float, help="coefficient for first-moment estimate")
    p.add_argument("--beta2", default=0.99999, type=float, help="coefficient for Hessian estimate")
    p.add_argument("--hess_init", default=0.5, type=float, help="initial Hessian value")
    p.add_argument("--cand_curvature", default=4e-6, type=float, help="candidate curvature value")
...
def build_model(args, device):
    ...
    input_size = INSIZE[args.dataset]

    try:
        model = MODELS[args.arch](num_classes, input_size).to(device)
        modelargs, modelkwargs = (num_classes, input_size), {}
    except TypeError:
        model = MODELS[args.arch](num_classes).to(device)
        modelargs, modelkwargs = (num_classes,), {}

    return model, modelargs, modelkwargs
...
if __name__ == "__main__":
    ...
    if args.resume:
        ...
        startepoch = 0
        model, modelargs, modelkwargs = build_model(args, device)
