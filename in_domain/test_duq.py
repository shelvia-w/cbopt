...
def get_args():
    ...
    p.add_argument("arch", type=str, choices=STANDARDMODELS, help="model architecture")
...
def do_evalbatch_duq(batchinput, model):
    ...
    loss = torch.nn.functional.nll_loss(prob.clamp_min(1e-12).log(), target)
...
if __name__ == "__main__":
    ...
    for seed in range(args.seed_start, args.seed_end + 1):
        ...
        if "optimizer_state_dict" in ckpt and ckpt["optimizer_state_dict"] is not None:
            param_groups = ckpt["optimizer_state_dict"].get("param_groups", None)
            if param_groups:
