...
from common.models import SNGPMODELS
from common.dataloaders import TRAINDATALOADERS, TESTDATALOADER, NTRAIN, OUTCLASS, INSIZE
...
def get_args():
    ...
    p.add_argument("arch", choices=SNGPMODELS, help="model architecture")
    ...
    # SNGP-specific model hyperparameters
    p.add_argument("--use_spec_norm", action="store_true", help="enable spectral normalization on hidden layers")
    p.add_argument("--spec_norm_iteration", default=1, type=int, help="number of power iterations for spectral normalization")
    p.add_argument("--spec_norm_bound", default=0.95, type=float, help="spectral norm bound")
    p.add_argument("--gp_input_dim", default=-1, type=int, help="optional frozen random projection dimension before GP layer; -1 disables it")
    p.add_argument("--gp_hidden_dim", default=1024, type=int, help="number of random features in GP layer")
    p.add_argument("--gp_scale", default=1.0, type=float, help="GP kernel scale / length-scale parameter")
    p.add_argument("--gp_bias", default=0.0, type=float, help="output bias init for GP layer")
    p.add_argument("--gp_input_normalization", action="store_true", help="apply LayerNorm before random Fourier features")
    p.add_argument("--gp_random_feature_type", default="orf", choices=["orf", "rff"], help="type of random features")
    p.add_argument("--gp_cov_discount_factor", default=-1.0, type=float, help="covariance momentum; use -1 for exact accumulation")
    p.add_argument("--gp_cov_ridge_penalty", default=1.0, type=float, help="ridge penalty for GP precision matrix")
    p.add_argument("--gp_mean_field_factor", default=math.pi / 8.0, type=float, help="mean-field logit correction factor")
    p.add_argument("--gp_output_init_std", default=0.01, type=float, help="std for GP output weight init")
...
if __name__ == "__main__":
    ...
    train_loader, val_loader = TRAINDATALOADERS[args.dataset](
        args.data_dir, args.tvsplit, args.workers, (device != torch.device("cpu")), args.tbatch, args.vbatch
    )
    test_loader = TESTDATALOADER[args.dataset](
        args.data_dir, args.workers, (device != torch.device("cpu")), args.tbatch
    )

    print(f"datasize {int(NTRAIN[args.dataset] * args.tvsplit)}, paramsize {sum(p.nelement() for p in model.parameters())}")
    ...
    for e in range(args.epochs):
        ...
        if args.warmup > 0 and e == args.warmup:
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, eta_min=args.lr_final, T_max=args.epochs)
        ...
        do_epoch(train_loader, TrainBatchSNGP(final_epoch), log_ece, device, model=model, optimizer=optimizer)
