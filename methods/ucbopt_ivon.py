"""IVON with a fixed candidate-curvature subtracted from the update denominator."""
...
class uCBOptIVON(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        if cand_curvature > weight_decay:
            raise ValueError(
                f"cand_curvature must be <= weight_decay "
    ...
    def _new_hess(method, hess, avg_nxg, avg_gsq, pg_slice, ess, beta2, wd) -> Tensor:
        f = uCBOptIVON._get_nll_hess(method, hess + wd, avg_nxg, avg_gsq, pg_slice, ess)
        return beta2 * hess + (1.0 - beta2) * f + \
            (0.5 * (1 - beta2) ** 2) * (hess - f).square() / (hess + wd)

    @staticmethod
    def _new_param_averages(
        param_avg, hess, momentum, lr, wd, cand, clip_radius, debias
    ) -> Tensor:
        return param_avg - lr * torch.clip(
            (momentum / debias + wd * param_avg) / (hess + wd - cand),
            min=-clip_radius,
            max=clip_radius,
        )
